...
std::optional<Intent> IntentEngine::parse_rule(const std::string& raw) {
    ...
    if (one_of(c, {"pest", "pests", "peststatus"})) { set("pest"); return intent; }
    ...
    if (one_of(c, {"weather", "forecast"})) { set("weather"); return intent; }
    if (one_of(c, {"inspect", "check"})) { set("inspect"); return intent; }
