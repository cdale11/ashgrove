...
| Pre-existing compiler warnings (2.1/2.2 soil + water-table code): 84 distinct warning sites in `src/main.cpp` (`-Wconversion`, `-Wsign-conversion`, `-Wshadow`, `-Wfloat-conversion`). ROADMAP 2.3 code itself is warning-clean. | Open | Batch-fix in a dedicated cleanup pass before pushing further milestones (SOP §3.4). |
