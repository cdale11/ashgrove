...
|     | **DONE 2026-08-19** (BuildingState gains rot/erosion/stress/fire_risk/fire_intensity/material/has_basement_hatch; Cell gains building_id/fire_intensity/fire_fuel/fire_risk; InvSlot gains durability/max_durability; deterministic daily `tick_structural_physics`: rot/erosion CA (material-dependent, moisture-driven, neighbor spread), foundation stress fields (CSP load distribution from weight/foundation/erosion/saturation), fire risk assessment (fuel+dryness+wind), fire spread CA (fuel+wind+humidity deterministic spread, basement hatch escape); tool wear grammar (durability per type: hoe 200, axe 300, pickaxe 350, scythe 180, watering can 150; wear on use: hoe 2, axe 3, pickaxe 4, scythe 2, can 1; repair at blacksmith); commands: `inspect`/`check` (building report), `toolrepair`/`fixtool` (blacksmith), `fire`/`ignite` (manual ignition), `structural` (valley report); intent rule verbs for all; serialization + lazy init; zero-warning build; intent regression 26/30 held) |     |     |
...
|     | **DONE 2026-08-19** (Wildlife struct gains metabolism Petri nets: hunger/thirst/energy/body_temp/circadian/age/life_stage; disease contact graph: transmission/recovery/immunity; aging L-systems: infant/juvenile/adult/senior stages, reproduction/gestation/birth, senior mortality; social graph rewriting: herd/pack formation with alpha/territory/cohesion, bond formation/decay, territorial disputes; deterministic daily `tick_creature_metabolism/disease/aging/social`; commands: `creature` (census/report), `herd` (pack details), `disease` (valley report); intent rule verbs: `creature`/`wildlife`/`animal`, `herd`/`pack`, `disease`/`sickness`; serialization + lazy init; zero-warning build; intent regression 26/30 held) |     |     |
| 2.9 | **8.5 Terrain & ecological change** — flood CA, river migration, erosion CA, fire spread, succession, soil degradation | 2.8 | 3.2 |
|     | **DONE 2026-08-19** (Cell gains terrain fields: flood_depth/duration, elevation, slope, sediment_depth, erosion_rate, compaction, salinity, nutrient_depletion, structure, succession_stage, disturbance_timer; deterministic daily `tick_terrain_ecology`: flood CA from rivers/rain with elevation-driven spread; river migration via bank erosion/deposition; erosion CA with slope-driven sediment transport; fire spread CA with wind/humidity/slope; succession stages 0-5 with disturbance reset; soil degradation: compaction/salinity/nutrient depletion/structure loss; commands `terrain`/`flood`/`erosion`/`succession`/`soil` + intent verbs; serialization + lazy init; zero-warning build; intent regression 26/30 held) |     |     |
| 2.10 | **Hidden State Persistence** — serialize all cognitive hidden states (RNN/SSM latent vectors, memories, adapted weights) to `save.json` per agent/aggregate; restore on load; discard on `newgame`. Applies to 7 important NPCs, 5 villagers, 6 aggregate minds. **DONE 2026-08-19** (CognitiveCore `save`/`load` per agent to `data/npc_cognitive_state/` (drives, memories, social graph, goals, causal traces); `CognitiveRegistry::get()` for lookup; VillageMind/EconomyMind/CultureMind/ValleyMind gain `to_json`/`from_json` + file `load`/`save` (collective emotions, schedule_bias, market_volatility, price_history, demand_shift, shared beliefs/fears, practice frequency, horror/fog/sanity/phantom pushes, memory ring); NatureMind `load`/`save` (from_json stub — state not restored, known limitation); `advance_day` saves cores (under `g_mutex`) + aggregate minds (outside lock) before `save_world`; boot loads all cores + minds after construction; `newgame` discards `data/npc_cognitive_state` + `data/*_mind.json`; zero-warning build; intent regression 26/30 held) | 1.7 (cognitive core wired) | 4.1 (narrative anchors need persistent minds) |
