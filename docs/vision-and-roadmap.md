...
>
> **Phase 8 status (in progress)**: Scaffolding shipped. Tier-0 rule fast path + Tier-1 LLM fallback (`IntentEngine`); command log collector (`data/cmdlog.jsonl`); dataset schema; golden eval set (30 cases) + `tools/eval_intents.py`; cloud teacher scaffold `tools/gen_dataset.py`. Teacher uses a **cloud model** (not gemma-4-E4B). Remaining: run teacher to build the distillation corpus, train the Qwen2.5-0.5B student (LoRA, CPU), swap student into Tier 1, and gate common commands on Tier 0. Tier-0 commands currently return in ~10 ms vs 10-30 s via the LLM.
